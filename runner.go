...
// screen)
func (r *Runner) ManualChoose() (done bool, change bool, direction string) {
	r.colorSelected =
	...
	if !r.colorSelected {
		if inpututil.IsKeyJustPressed(ebiten.KeyRight) {
			...
			change = true   // On retourne qu'une modification de sélection a été effectuée
			direction = "R" // On indique la direction vers laquelle s'effectue la sélection du runner
		} else if inpututil.IsKeyJustPressed(ebiten.KeyLeft) {
			r.colorScheme = (r.colorScheme + 7) % 8
			change = true   // On retourne qu'une modification de sélection a été effectuée
			direction = "L" // On indique la direction vers laquelle s'effectue la sélection du runner
		}
		return r.colorSelected, change, direction
	}
	return r.colorSelected, false, ""
}
